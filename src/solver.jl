function initialize!(p::ptr)
    ...
    p.uref[:, 1] = [0.0; 0.0; p.xref[idx.m, 1] * g]
    p.uref[:, p.K] = [0.0; 0.0; p.xref[idx.m, p.K] * g]
    q0 = p.x0[idx.q]
    qf = p.xref[idx.q, p.K]
    th = acos(dot(q0, qf))
    ...
    for k = 2:p.K-1
        p.xref[idx.r, k] = p.x0[idx.r] + ((p.xref[idx.r, p.K] - p.x0[idx.r]) / (p.K - 1)) * (k - 1)
        p.xref[idx.v, k] = p.x0[idx.v] + ((p.xref[idx.v, p.K] - p.x0[idx.v]) / (p.K - 1)) * (k - 1)
        ...
        p.xref[idx.w, k] = p.x0[idx.w] + ((p.xref[idx.w, p.K] - p.x0[idx.w]) / (p.K - 1)) * (k - 1)
